...
export default function AttachedDocs({ docs, onDocClick }: AttachedDocsProps) {
  ...
  return (
    <div className="mt-6 w-full min-w-0 max-w-full overflow-hidden">
      ...
      <div className="scrollbar-thumb-rounded-[4px] scrollbar-track-rounded-[4px] overflow-x-auto pb-1 scrollbar scrollbar-h-2 scrollbar-track-light-200 scrollbar-thumb-light-400 hover:scrollbar-thumb-light-500 dark:scrollbar-track-dark-200 dark:scrollbar-thumb-dark-400 dark:hover:scrollbar-thumb-dark-500">
        <div className="inline-flex min-w-0 gap-3 pr-2">
          {docs.map((doc) => (
            <button
              key={doc.publicId}
              type="button"
              onClick={() => onDocClick(doc.publicId)}
              className="group flex w-[100px] shrink-0 flex-col items-center gap-1.5 rounded-lg border border-light-200 bg-light-50 p-3 transition-colors hover:border-light-400 hover:bg-light-100 dark:border-dark-200 dark:bg-dark-200 dark:hover:border-dark-400 dark:hover:bg-dark-300"
            >
